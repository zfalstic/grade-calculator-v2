...
export function parseInput(fieldsInput: IClass[], resultsInput: string[]) {
  ...
  for (let i = 0; i < fieldsInput.length; i++) {
    ...
    if (
      !isNaN(+curr.examWeight) &&
      curr.examWeight.charAt(0) === "0" &&
      curr.examWeight.charAt(1) === "."
    ) {
      ...
      curr.examWeight.charAt(curr.examWeight.length - 1) === "%" &&
      curr.examWeight.substring(0, curr.examWeight.length - 1)
    ) {
      currNum.examWeight =
        parseFloat(curr.examWeight.substring(0, curr.examWeight.length - 1)) /
        100;
    } else {
      errors.add(i + 1);
      continue;
